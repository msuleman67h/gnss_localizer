...
static void GNSSCallback(const geometry_msgs::Vector3StampedPtr &msg)
{
  //geo_pos_conv geo;
  ...
  bool northp;
  double x, y, z;
  UTMUPS::Forward(msg->vector.x, msg->vector.y, zone, northp, x, y);
  string zonestr = UTMUPS::EncodeZone(zone, northp); // Stores UTM Grid Zone. Flint is 17
  z = (double) msg->vector.z;
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber gnss_pose_subscriber = nh.subscribe("/filter/positionlla", 100, GNSSCallback);
